...
import { PicoColorKey, PicoFormatter, SshTunnelConfig } from './types';
...
const closeTunnel = (logger?: ViteDevServer['config']['logger']) => {
	const log = (color: PicoColorKey, message: string) => {
		const formatter = pc[color] as PicoFormatter;
...
export function sshTunnel(config: SshTunnelConfig): Plugin {
	...
	return {
		...
		configureServer(server) {
			...
			const log = (color: PicoColorKey, message: string) => {
				const formatter = pc[color] as PicoFormatter;
				logger.info(
					pc.magenta('  ➜') + pc.magenta('  tunnel: ') + formatter(message)
				);
			}
			...
			if (!exitHandlersAttached) {
				const gracefulShutdown = () => {
					if (isTunnelActive) {
						const logExit = (color: PicoColorKey, message: string) => {
							const formatter = pc[color] as PicoFormatter;
